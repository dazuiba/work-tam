...
class Query
  ...
  def self.create_by_testsuite(suite)
    ...
    # already set on this, see testsuites_controller#retrieve_query
  ...
  def sql_for_field(field, value, db_table, db_field, is_custom_filter)
	...
	def custom_filters(custom_fields)
      ...
      when "user"
      	options = { :type => :list, :values => field.possible_values(:project_id => project_id), :order => 20}
      when "date"
