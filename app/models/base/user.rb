...
class User < ActiveRecord::Base
  ...
  def safeget_config(*args)
  	current_hash = self[:config]||{}
  	args.each do |e|
  		current_hash = current_hash[e]||{}
  	end
  	current_hash
  end
  ...
  def auto_config
  	(self['config']||{}).reverse_merge(:line_id => 1)
  end
  ...
  def attach_ldap!
  	...
  	conn = LDAP::Conn.new("10.13.2.1", 3268)
...
private
  ...
  def pick_ldap_result(ret, key)
    return nil if ret.nil?||ret.first.nil?
  	r = ret.first[key]
  	r&&r.first
  end
end
