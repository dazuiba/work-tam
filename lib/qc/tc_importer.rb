module QC
  ...
  OPTIONS = {:validate => false, :timestamps => false}  
    ...
    def import_preview!(logger=STDOUT)
      ...
      qc_ids = qc_test.map{|e|self.project.id.to_s + e.ts_test_id.to_s}
      testcase_ids = Auto::Testcase.find_all_by_project_id(project_id).map{|e|e.id.to_s}
      Auto::Testcase.find(testcase_ids - qc_ids)
    end
        ...
        "用例树条目"=>"Auto::TestcaseCategory.count(:conditions=>{:project_id=>#{self.project.id}})") do
				...
				steps = select_all("dessteps", "where ds_test_id in #{convert_in_sql(qc_test.map{|e|e.ts_test_id}, "ds_test_id")}").map{|e|convert_step(e)}
